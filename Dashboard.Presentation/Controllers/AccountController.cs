...
using Dashboard.Presentation.Helpers;
...
namespace Dashboard.Presentation.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    return RedirectToLocal(String.IsNullOrEmpty(returnUrl) ? "/Admin/Dashboards" : returnUrl);
        ...
        public ActionResult Register(string code)
        {
            ...
            {
                HttpCookie userCookie = new HttpCookie("REF");
                userCookie.Value = code;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                user.AndroidLink = string.Format(Constant.BaseUrl, Constant.AndroidUrl, user.UserName, "Android") + "{0}";
                user.IOSLink = string.Format(Constant.BaseUrl, Constant.IOSUrl, user.UserName, "IOS") + "{0}";
